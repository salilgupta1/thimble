...
def oldlandingpage(request):
    ...
    if request.method == "POST":
        ...
        try:
            chimp.list_subscribe(os.environ['MAILCHIMP_LIST_ID'], email, {'FIRST': 'unit', 'LAST': 'tests'},
                                 double_optin=False, send_welcome=True)
        except:
            context[
                'error'] = "We're sorry, it seems there was an error. This may be because you are already signed up."
        else:
            context['success'] = "Thank you for your interest in Thimble!"
